...
Get detailed info/attributes for features 

gui for advanced features (spatial relationship, intersection, etc.)

Basic input validation for forms 
check for valid endpoint before submitting
check for valid sparql query before submitting
use ruby gem to check queries (gem sparql does not work with rails 3.2 [I think anyways])
Instead use sparqljs (npm install)
json stringify before api request

Openlayers-rails gem is no longer maintained
use rails-assets-openlayers gem instead
Replace with new version of openlayers 
error prevents scrolling and click-dragging 
