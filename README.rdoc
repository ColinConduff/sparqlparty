...
Implement: connect select sparql endpoint menus up to home.queries functions so that users can query from different sparql endpoints/datasets

Check whether user has specified different sparql endpoints, and do a federated query if necessary.

Refactor update tables function.  Does not parse all of the data in json response messages. 

Allow users to select the sparql endpoint that is used.  If the feature sets use different sparql endpoints, the spatial and binary relationship queries may need to use a federated query to query two separate sparql endpoints at the same time.

Regarding spatial relationship: List the resulting map layers/features in a dropdown list so that the user can select them on the map.
...
Refactor html code.  Use a for loop (with erb templating lang) in index.html.erb to minimize redundant html.  
...
Feature sets 1 and 2, spatial relationship results, and binary relationship results have different colored layers.

Added button to map that toggles expanding the map and hiding the sidebar (dropdown menus) or shrinking the map and showing the sidebar.
